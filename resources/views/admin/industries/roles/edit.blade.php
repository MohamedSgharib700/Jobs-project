            ...
            <div class="row">
                <div class="col-6">
                    <div class="page-header">
                        <h4 class="page-title">{{ trans('update_role') }}</h4>
                    </div>
                </div>
            </div>
            <!--page-header closed-->
                    ...
                    <div class="col-lg-12 col-xl-6 col-md-12 col-sm-12 mx-auto">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('admin.industry.roles.update', ['industry'=>$industry->id,'role' => $role->id]) }}" method="post" id="horizontal-validation" class="form-horizontal">
                                    @method("PATCH")
                                    @csrf
                                    <div class="form-group row">
                                        <label for="ar[name]" class="col-md-3 col-form-label">{{ trans('arabic_name') }}</label>
                                        <div class="col-md-9">
                                            <input type="text" class="form-control" name="ar[name]" id="ar[name]" value="{{ $role->translate('ar')->name }}">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="en[name]" class="col-md-3 col-form-label">{{ trans('english_name') }} </label>
                                        <div class="col-md-9">
                                            <input type="text" class="form-control" name="en[name]" id="en[name]" value="{{ $role->translate('en')->name }}">
                                        </div>
                                    </div>
