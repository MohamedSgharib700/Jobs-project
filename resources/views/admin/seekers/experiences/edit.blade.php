                                    ...
                                    <div class="tab-pane fade show active p-0" id="doc4" role="tabpanel" aria-labelledby="dec-tab4">
                                        <div class="row">
                                            <div class="col-lg-12 col-xl-6 col-md-12 col-sm-12 mx-auto">
                                                <div class="card">
                                                    <form action="{{ route('admin.seeker.experiences.update',['seeker'=>$seeker,'experiences' => $seekerExperiences]) }}" method="post">
                                                        @csrf
                                                        @method("Put")
                                                        <div class="card-header">
                                                        ...
                                                        <div class="card-body">
                                                            ...
                                                            <div class="form-group">
                                                                <select name="years_of_experience" id="years_of_experience" class="select2 col-md-12">
                                                                    ...
                                                                    @foreach(YearsExperiences::getList() as $key => $value)
                                                                        <option value="{{ $key }}" {{ old(" years_of_experience", $seekerExperiences->years_of_experience) == $key ? "selected":null }}>{{ $value }}</option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <label for="education_level">{{ trans('career_level') }}</label>
                                                            <div class="form-group">
                                                                <select class="select2 col-md-12" name="career_level" id="career_level">
                                                                    ...
                                                                    @foreach(CareerLevels::getList() as $key => $value)
                                                                        <option value="{{ $key }}" {{ old("career_level", $seekerExperiences->career_level) == $key ? "selected":null }}>{{ $value }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="previous_experience" class="mt-3">{{ trans('previous_experience') }}</label>
                                                                <input type="text" class="form-control" name="previous_experience" id="previous_experience" value="{{old('previous_experience', $seekerExperiences->previous_experience)}}" placeholder="{{ trans('previous_experience') }}">
                                                            </div>
                                                            <div class="row">
                                                                <div class="col-lg-6 col-md-12">
                                                                    <div class="form-group">
                                                                        <label for="previous_experience_from_date">{{ trans('previous_experience_from_date') }}</label>
                                                                            ...
                                                                            <input type="date" value="{{old('previous_experience_from_date', $seekerExperiences->previous_experience_from_date)}}" max="<?php echo date("Y-m-d"); ?>"class="form-control" name="previous_experience_from_date" id="previous_experience_from_date" value="{{ old('previous_experience_from_date',$seekerExperiences->previous_experience_from_date) }}" placeholder="{{ trans('previous_experience_from_date') }}">
                                                                ...
                                                                <div class="col-lg-6 col-md-12">
                                                                    <div class="form-group">
                                                                        ...
                                                                        <input type="date" value="{{old('previous_experience_to_date', $seekerExperiences->previous_experience_to_date)}}" max="<?php echo date("Y-m-d"); ?>"class="form-control" name="previous_experience_to_date" id="previous_experience_to_date" value="{{ old('previous_experience_to_date',$seekerExperiences->previous_experience_to_date) }}" placeholder="{{ trans('previous_experience_to_date') }}">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <div class="form-group">
                                                                    <select class="select2 col-md-12" name="education_level" id="education_level">
                                                                        ...
                                                                        @foreach(EducationLevel::getList() as $key => $value)
                                                                            <option value="{{ $key }}" {{ old("education_level", $seekerExperiences->education_level) == $key ? "selected":null }}>{{ $value }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <select multiple="multiple" class="multi-select" name="languages[]" id="languages">
                                                                @foreach($languages as $language)
                                                                    <option value="{{ $language->id }}" @if($seeker->languages->containsStrict('id', $language->id)) selected="selected" @endif>{{ $language->name }}</option>
                                                                @endforeach
                                                            </select>
                                                            ...
                                                            <select name="working_countries[]" id="working_countries" multiple="multiple" class="multi-select mb-3">
                                                                @foreach($countries as $country)
                                                                    <option value="{{ $country->id }}" @if($seeker->workingCountries->containsStrict('id', $country->id)) selected="selected" @endif>{{ $country->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
